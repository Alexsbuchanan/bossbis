...
this file. At **v0.1.2** the `PlayerVsNpcCalc.accuracy` (player attack rolls +
hit chance), `PlayerVsNpcCalc.defenceRoll` (getNPCDefenceRoll + defence
reductions), and `PlayerVsNpcCalc.maxMeleeHit` (getPlayerMaxMeleeHit +
getMinAndMax melee dispatch + getMaxHit) paths are ported, so `ScenarioPlayer`
...
float relative ≤1e-9), and the 6 plain melee `maxHit` rows RUN and PASS exactly
(osmumtens-fang 50, dragon-hunter-lance 58, obsidian 46, obsidian+salve 52,
obsidian+avarice 53, Duke demonbane 50). The 2 magic `maxHit`
rows still SKIP (`maxMagicHit` not-ported, v0.1.3), and the `blisterwood-flail`
...
exactly 14 rows run and 3 skip. The remaining `*Calc` / `HitDist` / `scaling/*` /
