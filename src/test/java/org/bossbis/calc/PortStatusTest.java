 ...
 * un-ported calc path). At v0.1.3 (Milestone 1) the {@code PlayerVsNpcCalc.accuracy},
 * {@code PlayerVsNpcCalc.defenceRoll}, {@code PlayerVsNpcCalc.maxMeleeHit}, and
 * {@code PlayerVsNpcCalc.maxRangedHit} paths are ported, so the 3 {@code maxAttackRoll} rows, the 3
 * {@code npcDefRoll} rows, the 2 {@code accuracy} (hit-chance) rows, the 6 plain melee {@code maxHit}
 * rows (3 seeded + 3 v0.1.2 coverage), and the 3 ranged {@code maxHit} rows (bofa L99 = 29, bofa L1 = 3,
 * comp-ogre-bow + brutal vs Zogre = 19) RUN (assert). The 2 magic {@code maxHit} rows still SKIP
 ...
 * Skip-count is exactly {@code total - 17}. This locks both that the attack-roll + defence-roll +
 * accuracy + melee/ranged-max-hit rows now have live assertions and that no other row leaked into the
 * asserting set prematurely.
 */
class PortStatusTest
{
	/**
	 * Number of corpus rows whose calc path is ported at v0.1.3 (Milestone 1): the 3 {@code maxAttackRoll}
	 ...
	 * ranged {@code maxHit} (maxRangedHit) rows (bofa L99/L1, comp ogre bow). The blisterwood row needs
	 * the distribution pipeline for its vampyre bonus and so skips until v0.1.4.
	 */
	private static final long PORTED_ROWS = 17;
	...
	void onlyThePortedRowsRunTheRestSkip()
	{
		...
		assertThat(wouldSkip)
			.as("at v0.1.3 the 3 maxAttackRoll + 3 npcDefRoll + 2 accuracy + 6 melee maxHit + 3 ranged maxHit rows run; the 2 magic maxHit + blisterwood (distribution) rows skip")
			.isEqualTo(rows.size() - PORTED_ROWS);

		long wouldRun = rows.size() - wouldSkip;
		assertThat(wouldRun)
			.as("exactly the 3 attack-roll + 3 defence-roll + 2 accuracy + 6 melee + 3 ranged max-hit rows assert at v0.1.3")
